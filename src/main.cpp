...
int main(int argc, char* argv[]) {
    if (argc != 7) {
        std::cerr << "Oczekiwano: " << argv[0] << " <teatr.csv> <miesiac.csv> <tydzien.csv> <dyspo.csv> <grafik_out.csv> <miesiac_out.csv>\n";
        return 1;
    }
    ...
    const char* dyspo_in = argv[4];
    const char* grafik_out = argv[5];
    const char* miesiac_out = argv[6];

    try {
        TydzienPracy* tydzienPracy = TydzienPracy::zacznijTydzienPracy(teatr_in, miesiac_in, tydzien_in, dyspo_in);
